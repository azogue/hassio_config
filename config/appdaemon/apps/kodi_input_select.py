    ...
    def _receive_kodi_result(self, event_id, payload_event, *args):
        ...
        if event_id == EVENT_KODI_CALL_METHOD_RESULT:
            if method == "VideoLibrary.GetRecentlyAddedMovies":
                ...
                labels = list(list(zip(*data))[0])
                if not self._last_values or not all(
                    map(lambda x: x in labels, self._last_values)
                ):
                ...
                data = [
                    (r["label"], ("CHANNEL", r["channelid"], None))
                    for r in values
                ]
                self._ids_options.update(dict(zip(*zip(*data))))
                labels = list(list(zip(*data))[0])
                ...
                try:
                    ...
                    self.persistent_notification(
                        "No recordings found: {}. Can't set recordings menu".format(
                            result
                        ),
                        title="KODI ERROR",
                ...
                data = [
                    (
                        "{}:{}".format(_date(r["starttime"]), r["label"]),
                        ("MOVIE", r["file"], None),
                    )
                    for r in values
                ]
    ...
    def _change_selected_result(self, entity, attribute, old, new, kwargs):
        if new != old:
            try:
                ...
                self.persistent_notification(
                    "Selection not found: '{}'. Sorry. Keys are: {}".format(
                        new, self._ids_options.keys()
                    ),
